...
import json 
import glob
from utils.CommonVariables import COMMON_SKIPPED_LABELES, COMMON_RUN_WITH_DSTL, COMMON_LLAMA2_SENTENCE_LABELS
...
def generate_single_label_data_sentence(entities_dict, dirPath, self_labeled_data_path, reducedLabeles):
    # Initialize list to store the promts
    prompt_data_set = []
    ...
    for document_path in glob.glob(dirPath):
        ...
        for doc_entry in document_data:
            ...
            for sentence in re.split(pattern, doc_text):
                ...
                for label in COMMON_LLAMA2_SENTENCE_LABELS:
                    labels_present = True
                    data_point = {}
                    response = []
                    contexts = []
    ...
    df_self_labeled_data = generate_single_label_df_from_additional_data(self_labeled_data_path, reducedLabeles)
...
def generate_single_label_df_from_additional_data(self_labeled_data_path, reducedLabeles):

    data_set = []
    file = open (self_labeled_data_path)
    obj = json.load(file)
    for single_label in COMMON_LLAMA2_SENTENCE_LABELS:
        for i in range (len(obj)):
            labels_present = True
            data_point = {}
            response = []
